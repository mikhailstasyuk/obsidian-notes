...
Using the [[Markov property]] and assuming that , we can find a probability of moving from state $S_i$ to $S_j$: $$a_{i, j}=P(q_t=S_j|q_{t-1}=S_i) \text{‎‎‎ ‎ ‎ ‎ ‎}1≤i,j≤N,$$where $q_t$ is the state of the system at time $t$, $S$ is a state,  