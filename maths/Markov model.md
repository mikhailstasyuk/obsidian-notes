...
Using the [[Markov property]], we can find a probability of the next state of the system by looking at its previous state: $$a_{i, j}=P(q_t=S_j|q_{t-1}=S_i) \text{ }1 $$  where $q_t$ is the state of the system at time $t$, $S$ is a state, 